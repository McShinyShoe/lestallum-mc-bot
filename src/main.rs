...
async fn main() -> anyhow::Result<()> {
    ...
    let bot = BotController::new()
        .with_activity(VecDeque::from([Activity::Say { message: "/msg McShinyShoe Activity Hello World".to_string() }]))
        .with_startup_commands(vec!["/msg McShinyShoe Startup Commands".to_string(), "/pvp on".to_string()])
